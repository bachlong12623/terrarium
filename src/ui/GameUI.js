...
const TOOLS = [
  ...
  { action: 'dex', label: 'Sổ cây', icon: '≡' },
  { action: 'settings', label: 'Cài đặt', icon: '…', quiet: true },
];
...
export class GameUI {
  ...
  updatePlantDetail() {
    ...
    q('humid').textContent = `Ẩm: ${Math.round(care.humid)}`;
    q('radiant').textContent = `Sáng: ${Math.round(care.radiant)}`;

    const status = q('status');
    status.innerHTML = plant.withered
      ? '<p class="status-bad">Cây đang héo — hãy cân bằng độ ẩm để hồi phục.</p>'
      : '';
  ...
  showPlantPicker() {
    ...
    if (t.plants.length >= MAX_PLANTS) {
      this.showToast('Bình đã đầy — thu hoạch cây trưởng thành trước');
      return;
    }
    if (t.seeds <= 0) {
      this.showToast('Hết hạt giống — thu hoạch cây Hoàn thiện');
      return;
    }
  ...
  showSettings() {
    this.showPanel('Cài đặt', `
      <div class="settings">
        ...
        <button class="setting-btn danger" id="reset-save">Xóa và bắt đầu lại</button>
        <p class="hint-sm">Game tự lưu trên thiết bị. Xuất file để chuyển sang thiết bị khác.</p>
      </div>
    `);
